...
// Sides are ordered like so: 0 is top right, then 1 is below and it loops around to 5 which is above 0
...
function findNeighbor(hex: Item, hexagons: Item[], cellSize: Vector2, side: HexSide): Item | undefined {
    const diffX = cellSize.x * (3 / 4);
    const diffY = cellSize.y / Math.sqrt(3);

    let horiz: (h: Item) => boolean;
    let vert: (h: Item) => boolean;
    if (side === 0) {
        horiz = (h: Item): boolean => floorEq(h.position.x - diffX, hex.position.x);
        vert = (h: Item): boolean => floorEq(h.position.y + diffY / 2, hex.position.y);
    }
    ...
    if (side === 2) {
        horiz = (h: Item): boolean => floorEq(h.position.x, hex.position.x);
...
function extractCurveVertices(hexagons: Item[], cellSize: Vector2): Vector2[] {
    ...
    const rightMost = hexagons.sort((a, b) => b.position.x - a.position.x || b.position.y - a.position.y)[0];
    ...
    do {
        ...
        if (neighbor) {
            ...
            vertex = getSideVertex(currentHex, cellSize, currentSide);
        }
...
export const flatHex = { extractCurveVertices };
